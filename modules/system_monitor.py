...
def on_task(request: gradio.Request, func, *args, **kwargs):
    ...
    task_id = _extract_task_id(*args)
    model_title = util.get_value_from_cookie("sd_model_checkpoint", request)

    print(f"----------TASK_ID: {task_id} BEGIN -----------------------")
    # inspect func args
    import inspect
    ...
    request_data = {
        'api': api_name,
        'task_id': task_id,
        'user': user.User.current_user(request).uid,
        'args': func_args,
...
def on_task_finished(request: gradio.Request, task_id: str, status: str, message: str, time_consumption: dict):
    monitor_addr = modules.shared.cmd_opts.system_monitor_addr
    system_monitor_api_secret = modules.shared.cmd_opts.system_monitor_api_secret
    if not monitor_addr or not system_monitor_api_secret:
        logger.error('system_monitor_addr or system_monitor_api_secret is not present')
        return
    print(f"----------TASK_ID: {task_id} FINISH ----------------------")

    request_url = f'{monitor_addr}/{task_id}'
    ...
    if resp.status_code < 200 or resp.status_code > 299:
        logger.error((f'update monitor log failed, status: monitor_log_id: {task_id}, {resp.status_code}, '
...
def _make_gpu_consumption(func_name, named_args, *args, **kwargs) -> dict:
    ...
    if func_name in ('modules.txt2img.txt2img', 'modules.img2img.img2img'):
        ...
        if source_image:
            ...
            source_img_size = (1, 1)
        scale_type = args[7]  # 0: scale by, 1: scale to
        scale_by = args[8]
        scale_to = {
            'width': args[9],
            'height': args[10],
        ...
        if extras_mode == 0:  # single image
            ...
            if scale_type == 0:  # scale by, need calculate resultSize for every image particularly
                ...
                result['image_sizes'] = [{'width': scale_to['width'], 'height': scale_to['height']} for _ in
                                         range(image_count)]
